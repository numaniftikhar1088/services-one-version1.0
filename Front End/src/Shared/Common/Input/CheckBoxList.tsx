...
const CheckBoxList = (props: any) => {
  ...
  const handleOptionChange = (e: any, option: any) => {
    ...
    if (option.value.toLowerCase() === "none") {
      if (checked) {
        setDisableAllOptions(true);
        // Clear all other selections and keep only "None"
        const currentValue = props.Inputs[props.index].fields[props.fieldIndex].defaultValue;
        ...
        props.Inputs[props.index].fields[props.fieldIndex].defaultValue =
          Array.isArray(valueArray) 
            ? valueArray.filter((i: any) => i.value === option.value)
            : [];
      } else {
        setDisableAllOptions(false);
      }
      ...
      >
        {
          <div
            className={
              props.required
                ? "fw-500 mb-5 required text-primary"
                : "fw-500 mb-5"
            }
          >
            {t(props?.displayFieldName)}
          </div>
        }
        <div className="row">
          {Array.isArray(props?.RadioOptions) && props?.RadioOptions?.map(
            (options: any) =>
              options?.isVisable &&
              (options.value === "NoAllergies" ||
                options.value === "No Allergies" || options.value.toLowerCase() === "none" ? (
                  ...
                  <div className="form__group form__group--checkbox mb-3">
                    ...
                    >
                      <input
                        ...
                        disabled={
                          props?.sectionId === 45 || props?.sectionId === 46 || props?.sectionId === 120
                            ? props.isEnable
                            : disableAllOptions
                        }
                      />
