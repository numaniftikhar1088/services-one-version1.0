...
const Radio2 = (props: any) => {
  ...
  const { t } = useLang();
  const defaultSetMapRef = useRef<{ [fieldName: string]: boolean }>({});
  ...
  // Add this new function before emptySectionOnBillingType
  const resetRepeatFields = () => {
    if (
      ...
      props?.Inputs[props?.index].fields[props?.fieldIndex].repeatFields.forEach(
        (i: any) => {
          i.defaultValue = "";
          i.selectedText = "";
        }
      );
      // Update setInputs in the reset function to ensure state is updated even if below code errors
      props?.setInputs(props?.Inputs);
    }
  };

  const emptySectionOnBillingType = async (controlOptionsDependentFieldsList: any) => {
    if (
      props?.Inputs[props?.index]?.fields[props?.fieldIndex]?.repeatFields[
        props?.repeatFieldIndex
      ]?.systemFieldName === "BillingType"
    ) {
      // Call the separate reset function
      resetRepeatFields();

      // Collect matchedFields from all sections, not just patientIndex
      const matchedFields: any[] = [];
      props.Inputs?.forEach((section: any) => {
        section?.fields
          ?.filter((field: any) =>
            controlOptionsDependentFieldsList?.some(
              (dep: any) =>
                dep.systemFieldName?.trim() === field.systemFieldName?.trim()
            )
          )
          .forEach((field: any) => {
            const matchedDep = controlOptionsDependentFieldsList.find(
              (dep: any) =>
                dep.systemFieldName?.trim() === field.systemFieldName?.trim()
            );
            ...
            matchedFields.push({
              ...
              dependencyAction: matchedDep?.dependencyAction
            });
          });
      });

      const existingNames = new Set(
        backup.map((item) => item.systemFieldName)
      );

      backup = matchedFields.filter(
        (item: any) => !existingNames.has(item.systemFieldName)
      );

      if (controlOptionsDependentFieldsList.length > 0) {
        // Process all fields across all sections
        const allBackup = JSON.parse(
          localStorage.getItem("Backup") || "[]"
        );
        props.Inputs?.forEach((section: any) => {
          section?.fields?.forEach((field: any) => {
            const matchedBackup = backup.find(
              (b: any) => b.systemFieldName === field.systemFieldName
            );
            const b = allBackup.find(
              (b: any) => b.systemFieldName === field.systemFieldName
            )

            if (!matchedBackup) return;

            if (matchedBackup.dependencyAction.toLowerCase() === "optional") {
              field.validationExpression = "";
            } else {
              field.validationExpression = b.validationExpression;
            }
          });
        });
      }
        ...
        props.Inputs?.forEach((section: any) => {
          section?.fields?.forEach((field: any) => {
            const matchedBackup = allBackup.find(
              (b: any) => b.systemFieldName === field.systemFieldName
            );

            if (!matchedBackup) return;

            field.validationExpression = matchedBackup.validationExpression;
          });
        });
      }
      props?.setInputs(props?.Inputs);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const fieldName = props?.systemFieldName;


      if (
        !fieldName ||
        touch ||
        defaultSetMapRef.current[fieldName] ||
        !props?.RadioOptions ||
      ...
      if (defaultSelectedOption) {
        const e = {
          target: {
            ...
            attributes: {
              "data-name": { value: fieldName },
              "data-id": { value: defaultSelectedOption?.id },
              "insuranceOptionsId": { value: defaultSelectedOption?.optionDataID },
            },
            checked: false,
          },
        };
        ...
        defaultSetMapRef.current[fieldName] = true;
        setTouch(true);
      }
    }, 1000); // wait 1 second

    return () => clearTimeout(timer); // cleanup
  }, [props.RadioOptions, props.systemFieldName]);
      ...
      >
        ...
        <div className="row g-2">
          {props?.RadioOptions?.map((choice: any) => {
            ...
            const isPhysicianSignAvailable = phySign;
                ...
                <label
                  ...
                  className={
                    props.name === "POCTestResult"
                      ...
                      : "col-md-6 fw-400 d-flex justify-content-start align-items-start"
                  }
                ...
                >
                  <input
                    ...
                    onChange={async (e: any) => {
                      ...
                      emptySectionOnBillingType(choice.controlOptionsDependentFieldsList);
                      ...
                      if (props.sectionId === 4) {
                        if (choice?.value.toLowerCase() === "sample not collected") {
