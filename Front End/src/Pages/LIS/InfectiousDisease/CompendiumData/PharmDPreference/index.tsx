...
import {
  GetAllPharmDPreferences,
  GetFacilityLookUpByPerformingLab,
  GetPanelLookUp,
  GetPharmDSelectionLookUp,
} from 'Services/InfectiousDisease/PharmDPreference';
...
import Row, { PharmDPreferenceRow } from './Row';
import useIsMobile from 'Shared/hooks/useIsMobile';

type InputChangeEvent = HTMLInputElement | HTMLSelectElement;

export enum defaultSearchQuery {
  performingLab = '',
  facility = '',
  panelName = '',
  pharmD = '',
...
const queryDisplayTagNames: StringRecord = {
  ...
  pharmD: 'PharmD Selection',
};

export default function PharmDPreference() {
  const { t } = useLang();
  ...
  const [rows, setRows] = useState<PharmDPreferenceRow[]>(() => []);
  ...
  let initialCompendiumPharmDSelection = {
    performingLabId: 0,
    preferenceId: 0,
    facilityId: 0,
    panels: [],
  ...
  const FacilityLookUp = (labId: number) => {
    GetFacilityLookUpByPerformingLab(labId)
      .then((res: AxiosResponse) => {
        setDropDownValues((preVal: any) => {
          return {
            ...preVal,
  ...
  const PanelLookUp = (labId: number, facilityId: number) => {
    GetPanelLookUp(labId, facilityId)
      .then((res: AxiosResponse) => {
        setDropDownValues((preVal: any) => {
          return {
            ...preVal,
  ...
  const PharmDSelectionLookUp = (facilityId: number) => {
    GetPharmDSelectionLookUp(facilityId)
      .then((res: AxiosResponse) => {
        setDropDownValues((preVal: any) => {
          return {
            ...preVal,
            PharmDSelectionList: res?.data,
          };
        });
      })
      .catch((err: any) => {
                  ...
                  onClick={() => {
                    if (!isAddButtonDisabled) {
                      setRows((prevRows: any) => [
                        {
                          ...initialCompendiumPharmDSelection,
                          rowStatus: true,
                        },
                        ...prevRows,
                      ]);
              ...
              >
                <TableHead>
                  <TableRow className="h-40px">
                    ...
                    <TableCell>
                      <input
                        id={`IDCompendiumDataPharmDPreferencePerformingLab`}
                        type="text"
                        name="performingLab"
                        className="form-control bg-white  min-w-100px w-100 rounded-2 fs-8 h-30px"
                        placeholder={t('Search ...')}
                        value={searchRequest.performingLab}
                    ...
                    <TableCell>
                      <input
                        id={`IDCompendiumDataPharmDPreferenceFacility`}
                        type="text"
                        name="facility"
                        className="form-control bg-white  min-w-100px w-100 rounded-2 fs-8 h-30px"
                        placeholder={t('Search ...')}
                    ...
                    <TableCell>
                      <input
                        id={`IDCompendiumDataPharmDPreferencePanelName`}
                        type="text"
                        name="panelName"
                        className="form-control bg-white  min-w-100px w-100 rounded-2 fs-8 h-30px"
                        placeholder={t('Search ...')}
                    ...
                    <TableCell>
                      <input
                        id={`IDCompendiumDataPharmDPreferencePharmD`}
                        type="text"
                        name="pharmD"
                        className="form-control bg-white  min-w-100px w-100 rounded-2 fs-8 h-30px"
                        placeholder={t('Search ...')}
                        value={searchRequest.pharmD}
                        onChange={onInputChangeSearch}
                        onKeyDown={handleKeyPress}
                      />
                    </TableCell>
                  ...
                  <TableRow className="h-30px">
                    ...
                    <TableCell sx={{ width: 'max-content' }}>
                      <div
                        onClick={() => handleSort('performingLab')}
                        ...
                        <div className="d-flex justify-content-center align-items-center mx-4 mr-0">
                          <ArrowUp
                            CustomeClass={`${
                              ...
                              sort.clickedIconData === 'performingLab'
                                ? 'text-success fs-7'
                                : 'text-gray-700 fs-7'
                            }  p-0 m-0 "`}
                          />
                    ...
                    rows.map((item: any, index) => {
                      return (
                        <Row
                          ...
                          PharmDSelectionLookUp={PharmDSelectionLookUp}
