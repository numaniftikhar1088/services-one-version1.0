  ...
  const handlesave = async () => {
    try {
      if (!row.storageType || !row.storageType.trim()) {
        toast.error(t("Storage Type cannot be empty."));
        return;
      }

      const payload = {
        id: row.id || 0,
        storageTypeName: row.storageType,
        status: row.status,
      };

      const res = await StorageTypeSaveData(payload);
  ...
  return (
    <>
      <TableRow className="h-30px" key={row.id}>
            ...
            <div className="d-flex justify-content-center">
              ...
              >
                 <PermissionComponent
                   ...
                   permissionIdentifier="EditStorageType"
                 >
        ...
        <TableCell id={`BloodLisStorageTypeTitle_${row.id}`} sx={{ width: "max-content" }}>
          {row.rowStatus ? (
            <input
              id={`storageType`}
              type="text"
