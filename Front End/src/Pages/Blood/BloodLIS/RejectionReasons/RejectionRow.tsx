...
function RejectionRow({
  ...
  const handlesave = async () => {
    if (row.displayText.length === 0 && row.displayName.length === 0) {
      toast.error(t("Please Enter Rejection Name And Rejection Reason."));
      return;
    }
    if (row.displayName.length === 0) {
      toast.error("Please Enter Rejection Name.");
      return;
    }
  ...
  return (
    <>
      <TableRow className="h-30px">
          ...
          <div className="d-flex justify-content-center">
            {row?.rowStatus ? (
              <div className="gap-2 d-flex">
                <button
                  id={`BloodLisSettingRejectionReasonSave`}
                  onClick={() => {
                    handlesave();
                  }}
                ...
                <button
                  id={`BloodLisSettingRejectionReasonCancel`}
                  onClick={() => {
                    getCannedCommentsRejectionRecords();
                    handleClose("dropdown2");
                    if (row?.id != 0) {
              ...
              <div className="rotatebtnn">
                <DropdownButton
                  ...
                  key="end"
                  id={`BloodLisSettingRejectionReason3Dots_${row.id}`}
                  drop="end"
                ...
                >
                  <PermissionComponent
                    ...
                    permissionIdentifier="EditRejectionReason"
                  >
                    <Dropdown.Item
                      id={`BloodLisSettingRejectionReasonEdit`}
                      className="w-auto"
                      eventKey="2"
                      onClick={() => {
                        getValues(row);
                  ...
                  <PermissionComponent
                    moduleName="Blood LIS"
                    pageName="LIS Setting"
                    permissionIdentifier="DeleteRejectionReason"
                  >
                    <Dropdown.Item
                      id={`BloodLisSettingRejectionReasonDelete`}
                      className="w-auto"
                      eventKey="2"
        ...
        <TableCell
          id={`BloodLisSettingRejectionReasonDisplayName_${row.id}`}
        ...
        <TableCell
          id={`BloodLisSettingRejectionReasonDisplayText_${row.id}`}
      ...
      >
        ...
        <BootstrapModal.Footer className="p-0">
          <button
            id={`BloodLisSettingRejectionReasonModalCancel_${row.id}`}
            type="button"
          ...
          <button
            id={`BloodLisSettingRejectionReasonModalDelete_${row.id}`}
            type="button"
