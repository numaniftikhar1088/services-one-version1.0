...
function ExpandInfo(props: any) {
  ...
  const getPatientReqOrder = async () => {
    let response = await RequisitionType.workLogExpandInfo(props.reqOrderId);
  ...
  return (
    <div className="row gap-5">
      <div className="d-flex gap-5">
        <div className="card shadow-sm rounded border border-warning w-100">
        ...
        <div className="card shadow-sm rounded border border-warning w-100">
          ...
          <div className="card-body py-md-4 py-3">
            ...
            <div className="d-flex justify-content-between">
              <p className="fw-bold">Provider</p>
              <p>{data?.facilityInfo?.providerName}</p>
            </div>
            <div className="d-flex justify-content-between">
              <p className="fw-bold">NPI</p>
          ...
          <div className="card-body py-md-4 py-3">
            {data?.testingInfos?.map((info: TestingInfo, index: number) => (
              <div
                ...
                key={index}
              >
                <p className="fw-bold">{info.testName}</p>
                  ...
                  >
                    ...
                    <TableBody>
                      {data?.statusUpdates?.length ? (
                        data?.statusUpdates?.map((item: any) => (
                          <TableRow className="h-40px">
                            <TableCell>{item?.status}</TableCell>
                            <TableCell>{item?.stausNote}</TableCell>
                          </TableRow>
                        ))
                      ) : (
                        <NoRecord />
...
// card shadow-sm rounded border border-warning mt-3 col-xl-5 col-lg-5 col-md-12 col-sm-12
