...
namespace TrueMed.Business.Services.Lab
{
    public class LaboratoryManagement : ILaboratoryManagement
    {
        ...
        public LaboratoryManagement(MasterDbContext dbContext, IConnectionManager connectionManager)
        {
            this._dbContext = dbContext;
        ...
        public List<LabBaseViewModel> GetAllLabsList(params int[] exceptLabIds)
        {
            List<LabBaseViewModel> labs;
            
              var labCacheResp=_cacheManager.Get<List<LabBaseViewModel>>("AllTruemedLabs");
            if (!labCacheResp.IsSuccess || labCacheResp.Value == null || labCacheResp.Value.ToList().Count == 0)
            {

                labs = (from pri in _dbContext.TblLabs
                        join refer in GetAllDirectors()
                        on pri.LabId equals refer.LabId
                        select new LabBaseViewModel
                        {
                ...
                _cacheManager.Set<List<LabBaseViewModel>>("AllTruemedLabs", labs);
            }
            else
            {
                labs = labCacheResp.Value.ToList();
            }
            if (exceptLabIds != null && exceptLabIds.Count() > 0)
            {
                labs = labs.Where(x => !exceptLabIds.Contains(x.Id ?? 0)).ToList();
            }
            return labs;
        }
        ...
        public IQueryable<TrueMed.Domain.Model.Laboratory.LabUserViewModel> GetAllLabsUsers()
        {
            return _dbContext.TblLabUsers.Select(x => new TrueMed.Domain.Model.Laboratory.LabUserViewModel
            {
        ...
        public async Task<ICollection<LaboratoryBriefInfoViewModel>> GetLabBriefInfoByUserIdAsync(string userId)
        {
            ...
            {
                labs = GetAllLabsList().Where(x => x.IsReferenceLab == false).Where(x => labIds.Contains(x.Id)).Select(x => new LaboratoryBriefInfoViewModel
                {
                    LabKey = x.LabKey,
                    LabName = x.LabName,
                    LabUrl = x.LabUrl,
            ...
            {
                labs = GetAllLabsList().Where(x => x.IsReferenceLab == false).Select(lab => new
