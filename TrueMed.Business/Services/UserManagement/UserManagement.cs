...
namespace TrueMed.Business.Services.UserManagement
{
    public partial class UserManagement : TrueMed.Business.Interface.IUserManagement
    {
        ...
        public static Microsoft.AspNet.Identity.PasswordHasher PasswordHasher
        {
            get
            {
                return new Microsoft.AspNet.Identity.PasswordHasher();
            }
        }
        ...
        public bool AddUserInLabs(string userId, params int[] labIds)
        {
            ...
            {
                _dbContext.TblLabUsers.Add(new TrueMed.Sevices.MasterEntities.TblLabUser
                {
                    UserId = userId,
                    IsActive = true,
                    IsDefault = false,
        ...
        {
            var user = _dbContext.TblUsers.Select(x => new { x.Id, x.Email }).FirstOrDefault(x => x.Email == email);
            if (user != null)
                return user.Id;
            else
                return string.Empty;
        ...
        public IdentityResult RegisterUser(ApplicationUser user, string? password)
        {
            ...
            var userModel = new TrueMed.Sevices.MasterEntities.TblUser()
            {
        ...
        public IdentityResult UpdateUser(ApplicationUser user, string? password)
        {
            ...
            if (ack > 0)
            {
                ...
                foreach (var LabId in user.LabIds)
                {
                    ...
                    if (isLabExist)
                    {
                        var labUser = new TrueMed.Sevices.MasterEntities.TblLabUser()
                        {
                            LabId = LabId,
                            IsActive = true,
                            IsDefault = false,
        ...
        public IQueryable<TrueMed.Domain.Models.Identity.Response.UserReponseViewModel> GetAllUsers(int? labId, params string[] exceptUserIds)
        {
            ...
            var users = (from user in _dbContext.TblUsers
                         ...
                         .Select(x =>
                 new TrueMed.Domain.Models.Identity.Response.UserReponseViewModel
                 {
                     Id = x.pri.Id,
                     ...
                     UserType = (Domain.Enums.UserType?)(UserType)x.pri.UserType,
                     ...
                     AdditionalInfo = new TrueMed.Domain.Models.Identity.Response.UserReponseViewModel.UserAdditionalInfo
                     {
        ...
        public IdentityResult UpdateUserAdditionalInfo(UserAdditionalInfo user)
        {
            ...
            {
                userAdditionalInfoObj = new  TrueMed.Sevices.MasterEntities.TblUserAdditionalInfo();
        ...
        public IdentityResult SetNewPassword(ApplicationUser user, string password, SecurityQuestions? securityQuestions = null)
        {
            ...
            return new IdentityResult(isAffected ? TrueMed.Domain.Model.Identity.Status.Success : Status.Failed, isAffected ? "Password Successfully Added" : "Something went wrong", "Email");
        ...
        public string GenerateRequestTicket(string userId, TicketType ticketType, TimeSpan? timeSpan)
        {
            ...
            var tokenRequest = new TrueMed.Sevices.MasterEntities.TblRequestToken()
            {
                CreateDate = DateTime.UtcNow,
        ...
        public IdentityResult DeleteUser(string id)
        {
            ...
            var status = TrueMed.Domain.Model.Identity.Status.Failed;
            var ack = _dbContext.SaveChanges();
            if (ack > 0)
            {
                status = TrueMed.Domain.Model.Identity.Status.Success;
        ...
        public RequestResponse SaveEncodedText(EncodedTextRequest encodedTextRequest)
        {
            ...
            _dbContext.TblRequisitionEncodedTexts.Add(tblRequisitionEncoded);
            var ack = _dbContext.SaveChanges();
            if (ack > 0)
            {
        ...
        public RequestResponse< EncodedTextResponse> GetEncodedText(string? key)
        {
            ...
            var recordByKey = _dbContext.TblRequisitionEncodedTexts.FirstOrDefault(x => x.Key == key);
            if (recordByKey != null)
            {
